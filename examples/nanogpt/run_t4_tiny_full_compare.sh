...
REPORT_DIR="${REPORT_DIR:-${REPO_DIR}/reports/t4-tiny-$(date +%Y%m%d-%H%M%S)}"

CKPT_BASELINE="${CKPT_BASELINE:-${SCRIPT_DIR}/out-t4-tiny-baseline/ckpt.pt}"
CKPT_HC="${CKPT_HC:-${SCRIPT_DIR}/out-t4-tiny-hc/ckpt.pt}"
CKPT_MHC="${CKPT_MHC:-${SCRIPT_DIR}/out-t4-tiny-mhc/ckpt.pt}"
...
if [[ "${RUN_TRAIN}" == "true" ]]; then
  ...
  bash "${SCRIPT_DIR}/run_t4_tiny_compare.sh"
else
...
if [[ "${RUN_BENCH}" == "true" ]]; then
  run_benchmark "baseline" "${CKPT_BASELINE}" "${SCRIPT_DIR}/config/train_fineweb10B_tiny_t4.py"
  run_benchmark "hc" "${CKPT_HC}" "${SCRIPT_DIR}/config/train_fineweb10B_hc_tiny_t4.py"
  run_benchmark "mhc" "${CKPT_MHC}" "${SCRIPT_DIR}/config/train_fineweb10B_mhc_tiny_t4.py"
...
python "${SCRIPT_DIR}/summarize_training_runs.py" \
  --runs \
  "baseline=${SCRIPT_DIR}/out-t4-tiny-baseline" \
  "hc=${SCRIPT_DIR}/out-t4-tiny-hc" \
  "mhc=${SCRIPT_DIR}/out-t4-tiny-mhc" \
