...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        if self.v_residual:
            if self.v_residual_constrained:
                self.lamb_logits = nn.Parameter(torch.zeros(2))
            else:
    ...
    def forward(self, x, vrl_state=None):
        ...
        if self.v_residual:
            ...
            if self.v_residual_constrained:
                lamb1, lamb2 = torch.softmax(self.lamb_logits, dim=0)
            else:
                lamb1, lamb2 = self.lamb1, self.lamb2
            v = vrl_state.mix(v, lamb1, lamb2)
